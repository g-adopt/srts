...
c-- Jeroen Ritsema
cp  program to compute xyz slices from .raw files
...
c set the odd structure to be zero: jeroen
      if (i_even .eq. 2) then
        write(6,*) ' only even degree structure'
        do i=0,lmax
          j = 2*i+1
          k1 = j*j+1
          k2 = (j+1)*(j+1)
          ...
          enddo
        enddo
c set the even structure to be zero: jeroen
      else if (i_even .eq. 1) then
        write(6,*) ' only odd degree structure'
        do i=0,lmax
          j = 2*i
          k1 = j*j+1
          k2 = (j+1)*(j+1)
