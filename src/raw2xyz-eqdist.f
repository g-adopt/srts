      program raw2xyz_eqdist_pk

cp    Program to calculate horizontal slice through an .sph model, at equidistant points
cp    Program from AD/JR - March 2013
cp    Now input in program instead of stdin
      ...
      character*200 rawfl,xyzfl
      dimension d0(MXLENY),d1(MXLENY,NTMX),rotv(MXLENY),dr(MXLENY)
      dimension x(MXLENY)
      dimension xlst(NTMX),indxlst(NTMX)
...
c      call chekcl('| :r:2:Input .raw model file and output xyz file'
c     1          //'|-d  :r:1: gridding angle '
c     1          //'|-l  :o:2: lmin lmax'
c     1          //'|-lst:o:1: starting gridding longitude (0 default)'
c     1          //'|-f  :o:1: multiplication factor'
c     1          //'|')
...
c      lmn=inunx('-l',1,lf)
c      lmx=inunx('-l',2,lf)
c      if(lmx.eq.0) lmx=MXLH
c      rlst=reunx('-lst',1,lf)
c      ils=1
c      if(lf.eq.-1) ils=0
...
c     loop over latitudes
      do it=1,ntc
       ...
       indxlst(it)=nint(1./cos(xlat*dg2rad))
...
c       calculate model value
        ...
        xcheck=real(indxl)/real(indxlst(j))
        xcount=0.0
        do ic=1,360
        if(xcount.eq.xcheck) then   
        
        write(23,'(2f7.2,x,g15.5)') xlong2,xlst(j),fm*100.*xm
